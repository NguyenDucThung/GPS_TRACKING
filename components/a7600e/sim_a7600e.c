...
// Bộ đệm tĩnh lưu trữ chuỗi JSON tọa độ để trả về cho hàm main (Khởi tạo mặc định 0.0, 0.0)
static char g_gps_json_buf[128] =
"{\"latitude\":0.000000,\"longitude\":0.000000,\"status\":\"NO_FIX\"}";
...
// Hàm đọc tọa độ GPS thật (Nếu chưa fix vệ tinh sẽ gán mặc định 0.0, 0.0)
...
if (len > 0)
{
    ...
    if (p)
    {
        // Nếu chưa fix được vệ tinh (chưa có sóng) -> Bắn tọa độ 0.0, 0.0
        if (strstr(p, ",,,,,,,") != NULL || strstr(p, "+CGNSSINFO: ,") != NULL)
        {
            ESP_LOGW(TAG, "GPS chưa fix vị trí -> Bắn tọa độ mặc định 0.0, 0.0");
...
{
    ESP_LOGE(TAG, "Lỗi mất kết nối dữ liệu khi truy vấn chip GPS");
    snprintf(g_gps_json_buf, sizeof(g_gps_json_buf), 
             "{\"latitude\":0.000000,\"longitude\":0.000000,\"status\":\"NO_FIX\"}");
}
...
// GIỮ NGUYÊN BẢN CODE TEST THÀNH CÔNG CỦA ÔNG THUẬN
